...
namespace an602\console\command\update;
...
class check extends \an602\console\command\command
{
	/** @var config */
	protected $config;
	...
	* Construct method
	*/
	public function __construct(user $user, config $config, ContainerInterface $an602_container, language $language)
	{
		$this->config = $config;
	...
	*/
	...
	protected function check_ext(InputInterface $input, SymfonyStyle $io, $stability, $recheck, $ext_name)
	{
		try
		{
			$ext_manager = $this->an602_container->get('ext.manager');
	...
	{
		$version_helper = $this->an602_container->get('version_helper');
	...
	*/
	...
	{
		/** @var \an602\extension\manager $ext_manager */
		$ext_manager = $this->an602_container->get('ext.manager');
