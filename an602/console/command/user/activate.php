...
class activate extends command
{
	 ...
	 */
	public function __construct(user $user, driver_interface $db, config $config, language $language, log_interface $log, manager $notifications, user_loader $user_loader, $an602_root_path, $php_ext)
	{
		$this->db = $db;
		$this->config = $config;
