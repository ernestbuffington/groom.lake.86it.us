...
class application extends \Symfony\Component\Console\Application
{
	...
	*/
	...
	/**
	* @var \an602\config\config Config object
	*/
	protected $config;

	/**
	* @var \an602\language\language Language object
	*/
	protected $language;
	...
	* @param \an602\language\language	$language	The user which runs the application (used for translation)
	* @param \an602\config\config		$config		Config object
	*/
	public function __construct($name, $version, \an602\language\language $language, \an602\config\config $config)
	{
	...
	*
	* @param \an602\di\service_collection	$command_collection	The console service collection
	*/
	public function register_container_commands(\an602\di\service_collection $command_collection)
	{
		$commands_list = array_keys($command_collection->getArrayCopy());
		foreach ($commands_list as $service_command)
		{
			// config_text DB table does not exist in AN602 prior to 3.1
			// Hence skip cron tasks as they include reparser cron as it uses config_text table
			if (an602_version_compare($this->config['version'], '3.1.0', '<') && strpos($service_command, 'cron') !== false)
			{
				continue;
			}
