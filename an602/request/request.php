...
*/
class request implements \an602\request\request_interface
{
	/**
	...
	*/
	protected $super_globals = array(
		\an602\request\request_interface::POST => '_POST',
		\an602\request\request_interface::GET => '_GET',
		\an602\request\request_interface::REQUEST => '_REQUEST',
		\an602\request\request_interface::COOKIE => '_COOKIE',
		\an602\request\request_interface::SERVER => '_SERVER',
		\an602\request\request_interface::FILES => '_FILES',
	);
	...
	*/
	...
	/**
	* @var	\an602\request\type_cast_helper_interface	An instance of a type cast helper providing convenience methods for type conversions.
	*/
	protected $type_cast_helper;

	/**
	* Initialises the request class, that means it stores all input data in {@link $input input}
	* and then calls {@link \an602\request\deactivated_super_global \an602\request\deactivated_super_global}
	*/
	public function __construct(\an602\request\type_cast_helper_interface $type_cast_helper = null, $disable_super_globals = true)
	{
		if ($type_cast_helper)
		{
		...
		$this->original_request = $this->input[\an602\request\request_interface::REQUEST];
		$this->input[\an602\request\request_interface::REQUEST] = $this->input[\an602\request\request_interface::POST] + $this->input[\an602\request\request_interface::GET];
	...
	* This is achieved by overwriting the super globals with instances of {@link \an602\request\deactivated_super_global \an602\request\deactivated_super_global}
	*/
	public function disable_super_globals()
	{
		if (!$this->super_globals_disabled)
		{
			foreach ($this->super_globals as $const => $super_global)
			{
				...
				$GLOBALS[$super_global] = new \an602\request\deactivated_super_global($this, $super_global, $const);
			}
	...
	* @param	string	$super_global	(\an602\request\request_interface::POST|GET|REQUEST|COOKIE)
	* 									Specifies which super global shall be changed
	*/
	public function overwrite($var_name, $value, $super_global = \an602\request\request_interface::REQUEST)
	{
	...
	*/
	public function variable($var_name, $default, $multibyte = false, $super_global = \an602\request\request_interface::REQUEST)
	{
	...
	*					the same as that of $default. If the variable is not set $default is returned.
	*/
	public function untrimmed_variable($var_name, $default, $multibyte = false, $super_global = \an602\request\request_interface::REQUEST)
	{
	 ...
	 * {@inheritdoc}
	 */
	public function raw_variable($var_name, $default, $super_global = \an602\request\request_interface::REQUEST)
	{
		$path = false;
	...
	* whatever).
	*
	* @param	string|array	$var_name		See \an602\request\request_interface::variable
	* @param	mixed			$default		See \an602\request\request_interface::variable
	*
	...
	*/
	public function server($var_name, $default = '')
	{
		...
		if ($this->is_set($var_name, \an602\request\request_interface::SERVER))
		{
			return $this->variable($var_name, $default, $multibyte, \an602\request\request_interface::SERVER);
		}
		else
		{
			$var = getenv($var_name);
	...
	*/
	...
	{
		return $this->variable($form_name, array('name' => 'none'), true, \an602\request\request_interface::FILES);
	}
	...
	*/
	...
	{
		return $this->is_set($name, \an602\request\request_interface::POST);
	}
	...
	*/
	public function is_set($var, $super_global = \an602\request\request_interface::REQUEST)
	{
	...
	*/
	public function variable_names($super_global = \an602\request\request_interface::REQUEST)
	{
	...
	*					the same as that of $default. If the variable is not set $default is returned.
	*/
	protected function _variable($var_name, $default, $multibyte = false, $super_global = \an602\request\request_interface::REQUEST, $trim = true)
	{
	...
	* {@inheritdoc}
	*/
	public function get_super_global($super_global = \an602\request\request_interface::REQUEST)
	{
