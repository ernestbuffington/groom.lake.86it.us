...
class content_visibility
{
	...
	*/
	public function __construct(\an602\auth\auth $auth, \an602\config\config $config, \an602\event\dispatcher_interface $an602_dispatcher, \an602\db\driver\driver_interface $db, \an602\user $user, $an602_root_path, $php_ext, $forums_table, $posts_table, $topics_table, $users_table)
	{
		$this->auth = $auth;
		$this->config = $config;
	...
	*/
	public function is_visible($mode, $forum_id, $data)
	{
		...
		*
		* @event core.an602_content_visibility_is_visible
		...
		*/
		...
		extract($this->an602_dispatcher->trigger_event('core.an602_content_visibility_is_visible', compact($vars)));
	...
	*/
	public function get_visibility_sql($mode, $forum_id, $table_alias = '')
	{
		...
		*
		* @event core.an602_content_visibility_get_visibility_sql_before
		...
		*/
		...
		extract($this->an602_dispatcher->trigger_event('core.an602_content_visibility_get_visibility_sql_before', compact($vars)));
	...
	*/
	public function get_forums_visibility_sql($mode, $forum_ids = array(), $table_alias = '')
	{
		...
		*
		* @event core.an602_content_visibility_get_forums_visibility_before
		* @var	string		where_sql							Extra visibility conditions. It must end with either an SQL "AND" or an "OR"
		* @var	string		mode								Either "topic" or "post" depending on the query this is being used in
		...
		*/
		...
		extract($this->an602_dispatcher->trigger_event('core.an602_content_visibility_get_forums_visibility_before', compact($vars)));
	...
	*/
	public function get_global_visibility_sql($mode, $exclude_forum_ids = array(), $table_alias = '')
	{
		...
		*
		* @event core.an602_content_visibility_get_global_visibility_before
		...
		*/
		...
		extract($this->an602_dispatcher->trigger_event('core.an602_content_visibility_get_global_visibility_before', compact($vars)));
	...
	*/
	public function set_post_visibility($visibility, $post_id, $topic_id, $forum_id, $user_id, $time, $reason, $is_starter, $is_latest, $limit_visibility = false, $limit_delete_time = false)
	{
		...
		extract($this->an602_dispatcher->trigger_event('core.set_post_visibility_before_sql', compact($vars)));
		...
		extract($this->an602_dispatcher->trigger_event('core.set_post_visibility_after', compact($vars)));
		return $data;
	}
	...
	*/
	public function set_topic_visibility($visibility, $topic_id, $forum_id, $user_id, $time, $reason, $force_update_all = false)
	{
		...
		extract($this->an602_dispatcher->trigger_event('core.set_topic_visibility_before_sql', compact($vars)));
		$sql = 'UPDATE ' . $this->topics_table . '
			SET ' . $this->db->sql_build_array('UPDATE', $data) . '
			WHERE topic_id = ' . (int) $topic_id;
		$this->db->sql_query($sql);
		...
		extract($this->an602_dispatcher->trigger_event('core.set_topic_visibility_after', compact($vars)));
		return $data;
	}
