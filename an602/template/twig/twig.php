...
use an602\template\exception\user_object_not_available;

/**
* Twig Template class.
*/
class twig extends \an602\template\base
{
	/**
	...
	* Constructor.
	*
	* @param \an602\path_helper $path_helper
	* @param \an602\config\config $config
	* @param \an602\template\context $context template context
	* @param \an602\template\twig\environment $twig_environment
	* @param string $cache_path
	* @param \an602\user|null $user
	* @param array|\ArrayAccess $extensions
	* @param \an602\extension\manager $extension_manager extension manager, if null then template events will not be invoked
	*/
	public function __construct(\an602\path_helper $path_helper, $config, \an602\template\context $context, \an602\template\twig\environment $twig_environment, $cache_path, \an602\user $user = null, $extensions = array(), \an602\extension\manager $extension_manager = null)
	{
		$this->path_helper = $path_helper;
		$this->an602_root_path = $path_helper->get_an602_root_path();
		...
		// Add admin namespace
		if ($this->path_helper->get_adm_relative_path() !== null && is_dir($this->an602_root_path . $this->path_helper->get_adm_relative_path() . 'style/'))
		{
			$this->twig->getLoader()->setPaths($this->an602_root_path . $this->path_helper->get_adm_relative_path() . 'style/', 'admin');
		}
	}
	...
	* 	Default: array('styles') (AN602's style directory)
	...
	*/
	public function set_style($style_directories = array('styles'))
	{
		...
		foreach ($style_directories as $directory)
		{
			...
			{
				$path = $this->an602_root_path . trim($directory, '/') . "/{$name}/";
		...
		// If we're setting up the main AN602 styles directory and the core
	...
	/**
	* Set custom style location (able to use directory outside of AN602).
	*
	* Note: Templates are still compiled to AN602's cache directory.
	*
	...
	*/
	public function set_custom_style($names, $paths)
	{
		...
		if ($this->extension_manager instanceof \an602\extension\manager)
		{
			$names[] = 'all';
	...
	* @return \an602\template\template|string if $return_content is true return string of the compiled handle, otherwise return $this
	*/
	...
	*/
	protected function get_template_vars()
	{
		...
		$vars = array_merge(
			...
			array(
				'definition'	=> new \an602\template\twig\definition(),
				'loops'			=> $context_vars, // To get loops
			)
		);

		if ($this->user instanceof \an602\user)
		{
			$vars['user'] = $this->user;
		}
