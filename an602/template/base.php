...
abstract class base implements template
{
	...
	*/
	protected function call_hook($handle, $method)
	{
		...
		if (!empty($an602_hook) && $an602_hook->call_hook(array('template', $method), $handle, $this))
		{
			if ($an602_hook->hook_return(array('template', $method)))
			{
				$result = $an602_hook->hook_return_result(array('template', $method));
				return array($result);
			}
		}
