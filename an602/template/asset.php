...
class asset
{
	...
	* @param \an602\path_helper $path_helper Path helper object
	* @param \an602\filesystem\filesystem $filesystem
	*/
	public function __construct($url, \an602\path_helper $path_helper, \an602\filesystem\filesystem $filesystem)
	{
	...
	*/
	public function set_path($path, $urlencode = false)
	{
		...
		$real_root_path = $this->filesystem->realpath($this->path_helper->get_an602_root_path()) . DIRECTORY_SEPARATOR;

		// If the asset is under the AN602 root path we need to remove its path and then prepend $an602_root_path
		if ($real_root_path && substr($path . DIRECTORY_SEPARATOR, 0, strlen($real_root_path)) === $real_root_path)
		{
			$path = $this->path_helper->get_an602_root_path() . str_replace('\\', '/', substr($path, strlen($real_root_path)));
		}
		else
		{
