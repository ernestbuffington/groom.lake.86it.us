...
use an602\language\exception\language_file_not_found;
...
class language_file_loader
{
	...
	/**
	 * @var \an602\extension\manager	Extension manager
	 */
	 ...
	 *
	 * @param \an602\extension\manager	$extension_manager	Extension manager
	 */
	public function set_extension_manager(\an602\extension\manager $extension_manager)
	{
	...
	public function load($component, $locale, &$lang)
	{
		...
		$path = $this->an602_root_path . 'language/';
	...
	protected function load_file($path, $component, $locale, &$lang)
	{
		...
		if (strpos($this->an602_root_path . $component, $path) === 0)
		{
			// Filter out the path
			$path_diff = str_replace($path, '', dirname($this->an602_root_path . $component));
