...
class config_php_file
{
	...
	*/
	...
	/**
	* The path to the config file. (Default: $an602_root_path . 'config.' . $php_ext)
	*
	* @var string
	*/
	...
	*/
	function __construct($an602_root_path, $php_ext)
	{
		$this->an602_root_path = $an602_root_path;
		$this->php_ext = $php_ext;
		$this->config_file = $this->an602_root_path . 'config.' . $this->php_ext;
	}
	...
	* Otherwise prepends an602\db\driver\ to the dbms to convert a 3.0 dbms
	...
	*/
	public function convert_30_dbms_to_31($dbms)
	{
		...
		if (strpos($dbms, 'an602\db\driver') === false && class_exists('an602\db\driver\\' . $dbms))
		{
			return 'an602\db\driver\\' . $dbms;
		}

		if (class_exists($dbms))
		{
			// Additionally we could check that $dbms extends an602\db\driver\driver.
			// http://php.net/manual/en/class.reflectionclass.php
			// Beware of possible performance issues:
			// http://stackoverflow.com/questions/294582/php-5-reflection-api-performance
			// We could check for interface implementation in all paths or
			// only when we do not prepend an602\db\driver\.

			/*
			$reflection = new \ReflectionClass($dbms);

			if ($reflection->isSubclassOf('an602\db\driver\driver'))
			{
				return $dbms;
			}
			*/
		...
		if (preg_match('/(an602\\\db\\\driver\\\)?mysql$/i', $dbms))
		{
			return 'an602\db\driver\mysqli';
		}
