...
class finder
{
	...
	*                           _ext_finder
	*/
	public function __construct(\an602\filesystem\filesystem_interface $filesystem, $an602_root_path = '', \an602\cache\service $cache = null, $php_ext = 'php', $cache_name = '_ext_finder')
	{
	...
	* @return \an602\finder This object for chaining calls
	*/
	public function set_extensions(array $extensions, $replace_list = true)
	{
		...
		{
			$this->extensions[$ext_name] = $this->an602_root_path . 'ext/' . $ext_name . '/';
		}
		return $this;
	}
	...
	*
	* @param string $core_path The path relative to an602_root_path
	* @return \an602\finder This object for chaining calls
	*/
	...
	*/
	...
	/**
	* Finds classes matching the configured options if they follow AN602 naming rules.
	*
	...
	* AN602 naming rules an incorrect class name will be returned.
	*
	...
	public function get_classes_from_files($files)
	{
		...
		foreach ($files as $file => $ext_name)
		{
			...
			if ($ext_name === '/' && preg_match('#^includes/#', $file))
			{
				...
				$classes[] = 'an602_' . str_replace('/', '_', $class);
			}
			else
			{
	...
	*/
	protected function find_with_root_path($cache = true, $is_dir = false, $extension_keys = false)
	{
		...
		foreach ($items as $item => $ext_name)
		{
			if ($extension_keys)
			{
				$result[$ext_name] = $this->an602_root_path . $item;
			}
			else
			{
				$result[] = $this->an602_root_path . $item;
			}
		}
	...
	*/
	public function find($cache = true, $is_dir = false)
	{
		...
		{
			$extensions['/'] = $this->an602_root_path . $this->query['core_path'];
		}
					...
					if ($file_info->isDir() == $is_dir)
					{
							...
							(!$directory || preg_match($directory_pattern, $relative_path)))
						{
							$files[] = array(
								...
								'ext_name'		=> $ext_name,
								'path'			=> str_replace(array(DIRECTORY_SEPARATOR, $this->an602_root_path), array('/', ''), $file_info->getPath()) . '/',
								'filename'		=> $filename,
							);
						}
					}
