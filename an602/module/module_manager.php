...
namespace an602\module;

use an602\module\exception\module_exception;
use an602\module\exception\module_not_found_exception;
...
class module_manager
{
	 ...
	 * @param \an602\db\driver\driver_interface		$db					Database driver
	 * @param \an602\extension\manager				$ext_manager		Extension manager
	 * @param string								$modules_table		Module database table's name
	 * @param string								$an602_root_path	Path to AN602's root
	 * @param string								$php_ext			Extension of PHP files
	 */
	public function __construct(\an602\cache\driver\driver_interface $cache, \an602\db\driver\driver_interface $db, \an602\extension\manager $ext_manager, $modules_table, $an602_root_path, $php_ext)
	{
		$this->cache				= $cache;
		$this->db					= $db;
	...
	{
		$directory = $this->an602_root_path . 'includes/' . $module_class . '/info/';
		$fileinfo = array();
		...
		foreach ($modules as $cur_module)
		{
			...
			// format. an602_acp_info_acp_foo needs to be turned into
			// acp_foo_info and the respective file has to be included
			// manually because it does not support auto loading
			$old_info_class_file = str_replace("an602_{$module_class}_info_", '', $cur_module);
