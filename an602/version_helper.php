...
class version_helper
{
	 ...
	 */
	public function __construct(\an602\cache\service $cache, \an602\config\config $config, \an602\file_downloader $file_downloader)
	{
		$this->cache = $cache;
		$this->config = $config;
	...
	{
		return an602_version_compare($version1, $version2, $operator);
	}
	...
	*/
	...
	/**
	 * Gets the latest extension update for the current AN602 branch the user is on
	 * Will suggest versions from newer branches when EoL has been reached
	 * and/or version from newer branch is needed for having all known security
	 * issues fixed.
	 *
	...
	public function get_ext_update_on_branch($force_update = false, $force_cache = false)
	{
		...
		// Get current AN602 branch from version, e.g.: 3.2
		...
		{
			// If branches are empty, it means the current AN602 branch is newer than any branch the
