...
namespace an602\report\controller;

use an602\exception\http_exception;
...
class report
{
	...
	/**
	 * @var \an602\report\report_handler_interface
	 */
	protected $report_handler;

	/**
	 * @var \an602\report\report_reason_list_provider
	 */
	protected $report_reason_provider;

	public function __construct(\an602\config\config $config, \an602\user $user, \an602\template\template $template, \an602\controller\helper $helper, \an602\request\request_interface $request, \an602\captcha\factory $captcha_factory, \an602\report\handler_factory $report_factory, \an602\report\report_reason_list_provider $ui_provider, $an602_root_path, $php_ext)
	{
		$this->config			= $config;
		$this->user				= $user;
	 ...
	 *
	 * Because of how AN602 organizes routes $mode must be set in the route config.
	 *
	...
	public function handle($id, $mode)
	{
		...
		$redirect_url = append_sid(
			$this->an602_root_path . ( ($mode === 'pm') ? 'ucp' : 'viewtopic' ) . ".{$this->php_ext}",
		...
		catch (\an602\report\exception\pm_reporting_disabled_exception $exception)
		{
			throw new http_exception(404, 'PAGE_NOT_FOUND');
		}
		catch (\an602\report\exception\already_reported_exception $exception)
		{
			switch ($mode)
			{
				case 'pm':
		...
		catch (\an602\report\exception\report_permission_denied_exception $exception)
		{
		...
		catch (\an602\report\exception\entity_not_found_exception $exception)
		{
			$message = $exception->getMessage();
			if (isset($this->user->lang[$message]))
			{
		...
		catch (\an602\report\exception\empty_report_exception $exception)
		{
			$error[] = $this->user->lang['EMPTY_REPORT'];
		}
		catch (\an602\report\exception\invalid_report_exception $exception)
		{
	...
	protected function assign_template_data($mode, $id, $reason_id, $report_text, $user_notify, $error = array(), $s_hidden_fields = '', $captcha = false)
	{
		...
		{
			case 'pm':
				$report_route = $this->helper->route('an602_report_pm_controller', array('id' => $id));
			break;
			case 'post':
				$report_route = $this->helper->route('an602_report_post_controller', array('id' => $id));
			break;
		}
	 ...
	 * Check CAPTCHA
	 *
	 * @param	object	$captcha	A AN602 CAPTCHA object
