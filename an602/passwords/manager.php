...
class manager
{
	...
	* @param \an602\passwords\helper	$helper				Passwords helper object
	* @param array						$defaults			List of default driver types
	*/
	public function __construct(\an602\config\config $config, $hashing_algorithms, helper $helper, $defaults)
	{
		$this->config = $config;
		$this->helper = $helper;
	...
	*
	* @param \an602\di\service_collection $hashing_algorithms
	*/
	...
	* @param bool $skip_an602_check True if AN6022 password check should be skipped
	*
	* @return bool True if password is correct, false if not
	*/
	public function check_combined_hash($password, $stored_hash_type, $hash, bool $skip_an602_check = false)
	{
		...
		$is_valid_an602_pass = false;
		if (!$skip_an602_check)
		{
			$is_valid_an602_pass = $this->check_combined_hash(md5($password), $stored_hash_type, $hash, true);
		}
		...
		return hash_equals($hash, $this->helper->combine_hash_output($data, 'hash', $password)) || $is_valid_an602_pass;
	}
}
