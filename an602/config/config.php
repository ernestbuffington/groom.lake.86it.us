...
class config implements \ArrayAccess, \IteratorAggregate, \Countable
{
	...
	*/
	...
	{
		trigger_error('Config values have to be deleted explicitly with the \an602\config\config::delete($key) method.', E_USER_ERROR);
	}
	...
	* @param  string $key       The configuration option's name
	* @param  string $new_value New configuration value
	* @throws \an602\exception\http_exception when config value is set and not equal to new_value.
	* @return bool              True if the value was changed, false otherwise.
	*/
	public function ensure_lock($key, $new_value)
	{
		...
		throw new \an602\exception\http_exception(500, 'Failure while aqcuiring locks.');
	}
