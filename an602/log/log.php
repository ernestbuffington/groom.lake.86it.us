...
*/
class log implements \an602\log\log_interface
{
	/**
	...
	* Constructor
	*
	* @param	\an602\db\driver\driver_interface	$db		Database object
	...
	*/
	public function __construct($db, $user, $auth, $an602_dispatcher, $an602_root_path, $relative_admin_path, $php_ext, $log_table)
	{
		$this->db = $db;
		$this->user = $user;
		$this->auth = $auth;
		...
		$this->an602_admin_path = $this->an602_root_path . $relative_admin_path;
		...
		* it will not work for the \an602\log\log object we create in common.php
		*/
	...
	*/
	public function get_logs($mode, $count_logs = true, $limit = 0, $offset = 0, $forum_id = 0, $topic_id = 0, $user_id = 0, $log_time = 0, $sort_by = 'l.log_time DESC', $keywords = '')
	{
		...
		$profile_url = ($this->get_is_admin() && $this->an602_admin_path) ? append_sid("{$this->an602_admin_path}index.{$this->php_ext}", 'i=users&amp;mode=overview') : append_sid("{$this->an602_root_path}memberlist.{$this->php_ext}", 'mode=viewprofile');
		...
		*/
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			$log_entry_data = array(
				...
				'viewforum'			=> ($row['forum_id'] && $this->auth->acl_get('f_read', $row['forum_id'])) ? append_sid("{$this->an602_root_path}viewforum.{$this->php_ext}", 'f=' . $row['forum_id']) : false,
		...
		*/
		...
		if (count($topic_id_list))
		{
			...
			{
				$log[$key]['viewtopic'] = (isset($topic_auth['f_read'][$row['topic_id']])) ? append_sid("{$this->an602_root_path}viewtopic.{$this->php_ext}", 't=' . $row['topic_id']) : false;
				$log[$key]['viewpost'] = (isset($topic_auth['f_read'][$row['topic_id']]) && $row['post_id']) ? append_sid("{$this->an602_root_path}viewtopic.{$this->php_ext}", 'p=' . $row['post_id'] . '#p' . $row['post_id']) : false;
				$log[$key]['viewlogs'] = (isset($topic_auth['m_'][$row['topic_id']])) ? append_sid("{$this->an602_root_path}mcp.{$this->php_ext}", 'i=logs&amp;mode=topic_logs&amp;t=' . $row['topic_id'], true, $this->user->session_id) : false;
			}
		}
	...
	*/
	protected function get_topic_auth(array $topic_ids)
	{
		...
		*
		* @event core.an602_log_get_topic_auth_sql_before
		...
		*/
		...
		extract($this->dispatcher->trigger_event('core.an602_log_get_topic_auth_sql_before', compact($vars)));
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			 ...
			 *
			 * @event core.an602_log_get_topic_auth_sql_after
			...
			extract($this->dispatcher->trigger_event('core.an602_log_get_topic_auth_sql_after', compact($vars)));
