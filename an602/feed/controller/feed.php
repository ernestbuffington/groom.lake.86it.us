...
namespace an602\feed\controller;

use an602\auth\auth;
use an602\config\config;
use an602\db\driver\driver_interface;
use an602\event\dispatcher_interface;
use an602\exception\http_exception;
use an602\feed\feed_interface;
use an602\feed\exception\feed_unavailable_exception;
use an602\feed\exception\unauthorized_exception;
use an602\feed\helper as feed_helper;
use an602\controller\helper as controller_helper;
...
class feed
{
	 ...
	 * @param user $user
	 * @param auth $auth
	 * @param dispatcher_interface $an602_dispatcher
	 * @param language $language
	 * @param string $php_ext
	 */
	public function __construct(Environment $twig, symfony_request $request, controller_helper $controller_helper, config $config, driver_interface $db, ContainerInterface $container, feed_helper $feed_helper, user $user, auth $auth, dispatcher_interface $an602_dispatcher, language $language, $php_ext)
	{
	...
	protected function send_feed_do(feed_interface $feed)
	{
		...
		while ($row = $feed->get_item())
		{
			 ...
			 */
			$vars = array('feed', 'row');
			extract($this->an602_dispatcher->trigger_event('core.feed_modify_feed_row', compact($vars)));
