...
abstract class base implements feed_interface
{
	 ...
	 * Constructor
	 *
	 * @param \an602\feed\helper					$helper		Feed helper
	 * @param \an602\config\config				$config		Config object
	 * @param \an602\db\driver\driver_interface	$db			Database connection
	 * @param \an602\cache\driver\driver_interface	$cache	Cache object
	 * @param \an602\user						$user		User object
	 * @param \an602\auth\auth					$auth		Auth object
	 * @param \an602\content_visibility			$content_visibility		Auth object
	 * @param \an602\event\dispatcher_interface	$an602_dispatcher		Event dispatcher object
	 * @param string								$phpEx		php file extension
	 */
	public function __construct(
		\an602\feed\helper $helper,
		\an602\config\config $config,
		\an602\db\driver\driver_interface $db,
		\an602\cache\driver\driver_interface $cache,
		\an602\user $user,
		\an602\auth\auth $auth,
		\an602\content_visibility $content_visibility,
		\an602\event\dispatcher_interface $an602_dispatcher,
		$phpEx
	)
	{
		$this->config = $config;
	...
	public function get_item()
	{
		if (!isset($this->result))
		{
			 ...
			 */
			$vars = array('sql_ary');
			extract($this->an602_dispatcher->trigger_event('core.feed_base_modify_item_sql', compact($vars)));
			$this->sql = $sql_ary;
			unset($sql_ary);
