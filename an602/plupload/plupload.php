...
class plupload
{
	...
	* @param \an602\mimetype\guesser $mimetype_guesser
	*/
	public function __construct($an602_root_path, \an602\config\config $config, \an602\request\request_interface $request, \an602\user $user, \bantu\IniGetWrapper\IniGetWrapper $php_ini, \an602\mimetype\guesser $mimetype_guesser)
	{
	...
	* @return null
	*/
	public function configure(\an602\cache\service $cache, \an602\template\template $template, $s_action, $forum_id, $max_files)
	{
	...
	*/
	...
	{
		return $this->request->header('X-AN602-USING-PLUPLOAD', false);
	}
	...
	*/
	 ...
	 *
	 * @param \an602\cache\service	$cache		Cache service object
	 * @param string				$forum_id	The forum identifier
	 *
	 * @return string
	 */
	public function generate_filter_string(\an602\cache\service $cache, $forum_id)
	{
		$groups = [];
		$filters = [];
	...
	*/
	...
	protected function temporary_filepath($file_name)
	{
		...
		return sprintf(
			...
			md5($file_name),
			\an602\files\filespec::get_extension($file_name)
		);
	}
	...
	*/
	protected function integrate_uploaded_file($form_name, $chunk, $file_path)
	{
		...
		if (!an602_is_writable($this->temporary_directory) || !move_uploaded_file($upload['tmp_name'], $tmp_file))
		{
			$this->emit_error(103, 'PLUPLOAD_ERR_MOVE_UPLOADED');
		}
	...
	*/
	...
	{
		$this->upload_directory = $this->an602_root_path . $this->config['upload_path'];
