...
class file_downloader
{
	 ...
	 *
	 * @throws \an602\exception\runtime_exception If data can't be retrieved and no error
	 *		message is returned
	 */
	public function get($host, $directory, $filename, $port = 80, $timeout = 6)
	{
		...
		)
		{
			...
			while (!@feof($socket))
			{
				...
				else
				{
					...
					{
						throw new \an602\exception\runtime_exception('FILE_NOT_FOUND', array($filename));
					}
				}
				...
				{
					throw new \an602\exception\runtime_exception('FSOCK_TIMEOUT');
				}
			}
			@fclose($socket);
		}
		else
		{
			...
			{
				throw new \an602\exception\runtime_exception('FSOCK_DISABLED');
			}
		}
