...
namespace an602\auth\provider\oauth;
...
use an602\auth\provider\base;
use an602\auth\provider\db;
use an602\auth\provider\oauth\service\exception;
...
/**
 * OAuth authentication provider for AN6023
 */
class oauth extends base
{
	...
	public function login($username, $password)
	{
		...
		if ($this->is_set_code($service))
		{
			...
			if (!empty($ban))
			{
				...
				$contact_link = an602_get_board_contact_link($this->config, $this->root_path, $this->php_ext);
	 ...
	 * 									{@see \an602\auth\provider\provider_interface::link_account}
	...
	protected function get_service($provider, token_storage $storage, $query)
	{
		...
		/** @see \an602\auth\provider\oauth\service\service_interface::get_service_credentials */
		$service_credentials = $this->service_providers[$service_name]->get_service_credentials();

		/** @see \an602\auth\provider\oauth\service\service_interface::get_auth_scope */
