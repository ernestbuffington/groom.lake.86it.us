...
class auth
{
	...
	*/
	...
	{
		global $db, $cache, $an602_dispatcher;
		...
		*/
		$vars = array('user_id');
		extract($an602_dispatcher->trigger_event('core.acl_clear_prefetch_after', compact($vars)));
...
/*					This is uncommented, because i suspect this being slightly wrong due to mixed permission classes being possible
	...
	*/
	...
	{
		global $db, $user, $an602_root_path, $phpEx, $an602_container;
		...
		if ($provider)
		{
			...
			if ($login['status'] == LOGIN_SUCCESS_LINK_PROFILE)
			{
				...
				$url = append_sid($an602_root_path . 'ucp.' . $phpEx, array_merge($params, $login['redirect_data']));
			...
			extract($an602_dispatcher->trigger_event('core.auth_login_session_create_before', compact($vars)));
