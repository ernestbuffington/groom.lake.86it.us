...
class path_helper
{
	...
	* Constructor
	*
	* @param \an602\symfony_request $symfony_request
	...
	*
	* This replaces $an602_root_path . some_url with
	...
	*/
	public function update_web_root_path($path)
	{
		...
		{
			$path = $this->an602_root_path . substr($path, strlen($web_root_path));
		}

		if (strpos($path, $this->an602_root_path) === 0)
		{
	...
	*/
	public function remove_web_root_path($path)
	{
		if (strpos($path, $this->get_web_root_path()) === 0)
		{
			...
			return $this->an602_root_path . $path;
		}
	...
	*/
	public function get_web_root_path()
	{
		...
		// Full request URI (e.g. AN602/app.php/foo/bar)
		$request_uri = $this->symfony_request->getRequestUri();

		// Script name URI (e.g. AN602/app.php)
		...
		*/
		...
		{
			return $this->web_root_path = $this->filesystem->clean_path('./../' . $this->an602_root_path);
		}
		...
		*/
		if ($path_info === '/')
		{
			return $this->web_root_path = $this->an602_root_path;
		}
		...
		*	../page.php			memberlist.php		./AN602/
		*	../sub/page.php		memberlist.php		./../AN602/
		*
		...
		*/
		if ($this->request->is_ajax() && $this->symfony_request->get('_referer'))
		{
			...
			return $this->web_root_path = $this->an602_root_path . $referer_web_root_path;
		}
		...
		/*
		* If the script name (e.g. AN602/app.php) does not exists in the
		* requestUri (e.g. AN602/app.php/foo/template), then we are rewriting
		...
		*/
		...
		// Prepend ../ to the an602_root_path as many times as / exists in path_info
		$this->web_root_path = $this->filesystem->clean_path(
			'./' . str_repeat('../', $corrections) . $this->an602_root_path
		);
		return $this->web_root_path;
	}
	...
	*/
	public function get_web_root_path_from_ajax_referer($absolute_referer_url, $absolute_board_url)
	{
		...
		if (strpos($absolute_referer_url, $absolute_board_url) === 0)
		{
			...
			return $this->an602_root_path . str_repeat('../', $corrections - 1);
		}
		...
		while (($dir_position = strpos($absolute_board_url, $referer_dir)) !== 0)
		{
			...
			// Just return an602_root_path if we reach the top directory
			if ($referer_dir === '.')
			{
				return $this->an602_root_path;
			}
		...
		// Add trailing slash
		return $this->an602_root_path . $fixed_root_path . '/';
	}
	...
	*/
	...
	public function get_valid_page($page, $mod_rewrite = false)
	{
		...
		// For a relative path, let's prefix it with $an602_root_path to point to the correct location,
		...
		if ($url_parts === false || empty($url_parts['scheme']) || empty($url_parts['host']))
		{
			...
			// even if it might not be in the AN602 root.
			...
			$page = $this->get_an602_root_path() . ltrim($page, '/\\');
		}
