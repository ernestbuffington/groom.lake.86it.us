...
class type_date extends type_base
{
	...
	*/
	public function __construct(\an602\request\request $request, \an602\template\template $template, \an602\user $user)
	{
	...
	public function get_excluded_options($key, $action, $current_value, &$field_data, $step)
	{
		if ($step == 2 && $key == 'field_default_value')
		{
			...
			if ($always_now == 1 || ($always_now === -1 && $current_value == 'now'))
			{
				...
				$current_value = 'now';
				$this->request->overwrite('field_default_value', $current_value, \an602\request\request_interface::POST);
			}
			else
			{
