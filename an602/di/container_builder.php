...
class container_builder
{
	...
	/**
	 * Defines a custom path to find the configuration of the container (default to $this->an602_root_path . 'config')
	 *
	 * @var string
	 */
	 ...
	 * Default to:
	 * 	array(
	 * 		'core.root_path', $this->an602_root_path,
	 ...
	 *
	 * @return \an602_cache_container|ContainerBuilder
	 */
		...
		try
		{
			...
			if ($this->use_cache)
			{
				...
				if ($config_cache->isFresh())
				{
					...
					$this->container = new \an602_cache_container();
	 ...
	 * Set custom parameters to inject into the container.
	 *
	 * @param \an602\config_php_file $config_php_file
	 * @return $this
	 */
	public function with_config(\an602\config_php_file $config_php_file)
	{
	...
	{
		return $this->config_path ?: $this->an602_root_path . 'config';
	}
	...
	{
		return $this->cache_dir ?: $this->an602_root_path . 'cache/' . $this->get_environment() . '/';
	}
	...
	protected function load_extensions()
	{
		if ($this->config_php_file !== null)
		{
			...
			$container_builder = new container_builder($this->an602_root_path, $this->php_ext);
			...
			$ext_container->register('cache.driver', '\\an602\\cache\\driver\\dummy');
 ...
 * by AN602 while loading the extensions.
 */
...
';
			foreach ($extensions as $ext_name => $path)
			{
				...
				{
					$extension_class = '\\an602\\extension\\di\\extension_base';
				}
	...
	protected function inject_dbal_driver()
	{
		...
		if (!empty($config_data))
		{
			if ($this->dbal_connection === null)
			{
				...
				/** @var \an602\db\driver\driver_interface $dbal_connection */
	...
	{
		$container_params = [
			'an602_root_path' => $this->an602_root_path,
