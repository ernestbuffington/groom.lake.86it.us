...
*/
class local extends \an602\avatar\driver\driver
{
	/**
	* {@inheritdoc}
	*/
		...
		if (!empty($avatar_list[$category]))
		{
			...
			while ($avatar_pos < $avatar_count)
			{
				...
				$template->assign_block_vars('avatar_local_row.avatar_local_col', array(
					'AVATAR_IMAGE'  => $this->an602_root_path . $this->config['avatar_gallery_path'] . '/' . $img['file'],
	...
	*/
	protected function get_avatar_list($user)
	{
		...
		{
			$avatar_list = array();
			$path = $this->an602_root_path . $this->config['avatar_gallery_path'];
