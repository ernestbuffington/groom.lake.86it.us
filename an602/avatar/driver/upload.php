...
*/
class upload extends \an602\avatar\driver\driver
{
	/**
	...
	* @param \an602\filesystem\filesystem_interface $filesystem AN602 filesystem helper
	* @param \an602\path_helper $path_helper AN602 path helper
	* @param \an602\event\dispatcher_interface $dispatcher AN602 Event dispatcher object
	* @param \an602\files\factory $files_factory File classes factory
	* @param \an602\cache\driver\driver_interface $cache Cache driver
	*/
	public function __construct(\an602\config\config $config, $an602_root_path, $php_ext, \an602\filesystem\filesystem_interface $filesystem, \an602\path_helper $path_helper, \an602\event\dispatcher_interface $dispatcher, \an602\files\factory $files_factory, \an602\cache\driver\driver_interface $cache = null)
	{
		$this->config = $config;
	...
	*/
	public function process_form($request, $template, $user, $row, &$error)
	{
		...
		* @var	\an602\files\filespec file	Instance of filespec class
	...
	*/
	public function delete($row)
	{
		...
		$prefix = $this->config['avatar_salt'] . '_';
		$ext = substr(strrchr($row['avatar'], '.'), 1);
		$filename = $this->an602_root_path . $destination . '/' . $prefix . $row['id'] . '.' . $ext;
	...
	*/
	...
	{
		return ($this->filesystem->exists($this->an602_root_path . $this->config['avatar_path']) && $this->filesystem->is_writable($this->an602_root_path . $this->config['avatar_path']) && (@ini_get('file_uploads') || strtolower(@ini_get('file_uploads')) == 'on'));
	}
}
