...
namespace an602\extension;

use an602\exception\runtime_exception;
use an602\file_downloader;
...
class manager
{
	...
	*/
	public function __construct(ContainerInterface $container, \an602\db\driver\driver_interface $db, \an602\config\config $config, \an602\filesystem\filesystem_interface $filesystem, $extension_table, $an602_root_path, $php_ext = 'php', \an602\cache\service $cache = null, $cache_name = '_ext')
	{
		$this->cache = $cache;
	...
	*/
	public function get_extension_path($name, $an602_relative = false)
	{
		...
		return (($an602_relative) ? $this->an602_root_path : '') . 'ext/' . $name . '/';
	}
	...
	* @return \an602\extension\extension_interface Instance of the extension meta class or
	*                     \an602\extension\base if the class does not exist
	*/
	public function get_extension($name)
	{
		...
		{
			return new \an602\extension\base($this->container, $this->get_finder(), $migrator, $name, $this->get_extension_path($name, true));
		}
	}
	...
	*
	* @param string $name The extension name
	* @return \an602\extension\metadata_manager Instance of the metadata manager
	*/
	public function create_extension_metadata_manager($name)
	{
		if (!isset($this->extensions[$name]['metadata']))
		{
			$metadata = new \an602\extension\metadata_manager($name, $this->get_extension_path($name, true));
	...
	*/
	...
	{
		$available = array();
		if (!is_dir($this->an602_root_path . 'ext/'))
		{
			return $available;
		}
		...
		$iterator = new \RecursiveIteratorIterator(
			new \an602\recursive_dot_prefix_filter_iterator(
				new \RecursiveDirectoryIterator($this->an602_root_path . 'ext/', \FilesystemIterator::NEW_CURRENT_AND_KEY | \FilesystemIterator::FOLLOW_SYMLINKS)
			),
	...
	*/
	public function all_configured($an602_relative = true)
	{
		$configured = array();
		foreach ($this->extensions as $name => $data)
		{
			...
			{
				unset($data['metadata']);
				$data['ext_path'] = ($an602_relative ? $this->an602_root_path : '') . $data['ext_path'];
	...
	*               database stored extension information as values
	*/
	public function all_enabled($an602_relative = true)
	{
		$enabled = array();
		foreach ($this->extensions as $name => $data)
		{
			if ($this->is_enabled($name))
			{
				$enabled[$name] = ($an602_relative ? $this->an602_root_path : '') . $data['ext_path'];
			}
		}
		return $enabled;
	}
	...
	*               database stored extension information as values
	*/
	public function all_disabled($an602_relative = true)
	{
		$disabled = array();
		foreach ($this->extensions as $name => $data)
		{
			...
			{
				$disabled[$name] = ($an602_relative ? $this->an602_root_path : '') . $data['ext_path'];
			}
		}
		return $disabled;
	}
	...
	*/
	public function is_available($name)
	{
		...
		catch (\an602\extension\exception $e)
		{
			return false;
		}
	}
	...
	*
	* @param \an602\extension\metadata_manager $md_manager The metadata manager for the version to check.
	...
	*/
	public function version_check(\an602\extension\metadata_manager $md_manager, $force_update = false, $force_cache = false, $stability = null)
	{
		...
		$version_helper = new \an602\version_helper($this->cache, $this->config, new file_downloader());
	...
	*/
	...
	/**
	* Instantiates a \an602\finder.
	*
	* @param bool $use_all_available Should we load all extensions, or just enabled ones
	* @return \an602\finder An extension finder instance
	*/
	public function get_finder($use_all_available = false)
	{
		$finder = new \an602\finder($this->filesystem, $this->an602_root_path, $this->cache, $this->php_ext, $this->cache_name . '_finder');
