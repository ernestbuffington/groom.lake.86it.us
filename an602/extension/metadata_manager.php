...
class metadata_manager
{
	...
	*/
	private function fetch_metadata_from_file()
	{
		...
		{
			throw new \an602\extension\exception('FILE_NOT_FOUND', array($this->metadata_file));
		}

		if (!($file_contents = file_get_contents($this->metadata_file)))
		{
			throw new \an602\extension\exception('FILE_CONTENT_ERR', array($this->metadata_file));
		}

		if (($metadata = json_decode($file_contents, true)) === null)
		{
			throw new \an602\extension\exception('FILE_JSON_DECODE_ERR', array($this->metadata_file));
		}
		...
		switch ($name)
		{
			...
			default:
				if (isset($fields[$name]))
				{
					...
					{
						throw new \an602\extension\exception('META_FIELD_NOT_SET', array($name));
					}

					if (!preg_match($fields[$name], $this->metadata[$name]))
					{
						throw new \an602\extension\exception('META_FIELD_INVALID', array($name));
					}
				}
			break;
		}
	...
	*/
	public function validate_authors()
	{
		...
		{
			throw new \an602\extension\exception('META_FIELD_NOT_SET', array('authors'));
		}
		...
		foreach ($this->metadata['authors'] as $author)
		{
			...
			{
				throw new \an602\extension\exception('META_FIELD_NOT_SET', array('author name'));
			}
		}
	...
	*/
	public function validate_dir()
	{
		...
		{
			throw new \an602\extension\exception('EXTENSION_DIR_INVALID');
		}
	...
	*/
	public function validate_require_phpbb()
	{
		...
		{
			throw new \an602\extension\exception('META_FIELD_NOT_SET', array('soft-require'));
		}
	...
	*/
	public function validate_require_php()
	{
		...
		{
			throw new \an602\extension\exception('META_FIELD_NOT_SET', array('require php'));
		}
