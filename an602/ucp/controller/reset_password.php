...
namespace an602\ucp\controller;

use an602\auth\auth;
use an602\config\config;
use an602\controller\helper;
...
class reset_password
{
	...
	/** @var array AN602 DB table names */
	...
	public function request()
	{
		...
		if ($submit)
		{
			...
			else
			{
				...
				$messenger->assign_vars([
						...
						'U_RESET_PASSWORD'	=> generate_board_url(true) . $this->helper->route('an602_ucp_reset_password_controller', [
							'u'		=> $user_row['user_id'],
							'token'	=> $reset_token,
						], false)
				]);
		...
		$this->template->assign_vars([
			'USERNAME'					=> $username,
			'EMAIL'						=> $email,
			'U_RESET_PASSWORD_ACTION'	=> $this->helper->route('an602_ucp_forgot_password_controller'),
		]);
	...
	public function reset()
	{
		...
		$this->template->assign_vars([
			...
			'U_RESET_PASSWORD_ACTION'	=> $this->helper->route('an602_ucp_reset_password_controller'),
