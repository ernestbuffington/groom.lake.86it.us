...
namespace an602\cron;

use an602\cron\task\wrapper;
use an602\routing\helper;
...
class manager
{
	...
	/**
	* Set of \an602\cron\task\wrapper objects.
	...
	*/
	public function __construct(ContainerInterface $an602_container, helper $routing_helper, $an602_root_path, $php_ext, $template)
	{
	...
	*
	* @param array|\Traversable $tasks		Array of instances of \an602\cron\task\task
	*/
	...
	*/
	public function load_tasks_from_container()
	{
		if (!$this->is_initialised_from_container)
		{
			...
			$tasks = $this->an602_container->get('cron.task_collection');
	...
	*
	* @return array		List of tasks which are ready to run (wrapped in \an602\cron\task\wrapper).
	*/
	...
	*/
	...
	/**
	* Wraps a task inside an instance of \an602\cron\task\wrapper.
	*
	* @param  \an602\cron\task\task 			$task The task.
	* @return wrapper	The wrapped task.
	*/
	public function wrap_task(\an602\cron\task\task $task)
	{
		return new wrapper($task, $this->routing_helper, $this->an602_root_path, $this->php_ext, $this->template);
	}
}
