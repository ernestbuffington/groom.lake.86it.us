...
namespace an602\install;

use an602\cache\driver\driver_interface;
use an602\di\ordered_service_collection;
use an602\install\exception\cannot_build_container_exception;
use an602\install\exception\installer_config_not_writable_exception;
use an602\install\exception\jump_to_restart_point_exception;
...
use an602\install\helper\iohandler\ajax_iohandler;
...
class installer
{
	...
	public function run()
	{
		...
		if ($this->install_config->get_task_progress_count() === 0)
		{
			...
			/** @var \an602\install\module_interface $module */
		...
		try
		{
			...
			else
			{
				...
				if (!isset($module) || !($module instanceof \an602\install\module\install_finish\module))
				{
