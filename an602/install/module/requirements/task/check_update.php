...
class check_update extends task_base
{
	 ...
	 */
	public function __construct(container_factory $container, filesystem $filesystem, config $config, iohandler_interface $iohandler, update_helper $update_helper, $an602_root_path, $php_ext)
	{
	...
	public function run()
	{
		...
		$update_files = array(
			$this->an602_root_path . 'install/update',
			$this->an602_root_path . 'install/update/index.' . $this->php_ext,
		);
		...
		$update_info = array();
		@include($this->an602_root_path . 'install/update/index.' . $this->php_ext);
		...
		if (!$this->update_helper->an602_version_compare($current_version, $update_version, '<'))
		{
		...
		if ($this->update_helper->an602_version_compare($update_version, $latest_version, '<'))
		{
