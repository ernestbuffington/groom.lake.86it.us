 ...
 */
class add_config_settings extends \an602\install\task_base
{
	/**
	 ...
	 * @param \an602\install\helper\config							$install_config		Installer's config helper
	...
	public function run()
	{
		...
		$current_time 	= time();
		$user_ip		= an602_ip_normalise($this->iohandler->get_server_variable('REMOTE_ADDR'));
		...
		if (!$this->filesystem->is_writable($this->an602_root_path . 'images/avatars/upload/'))
		{
			$sql_ary[] = 'UPDATE ' . $this->config_table . "
				SET config_value = '0'
