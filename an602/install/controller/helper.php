...
use an602\install\helper\navigation\navigation_provider;
use an602\language\language;
use an602\language\language_file_helper;
use an602\path_helper;
use an602\request\request;
use an602\request\request_interface;
use an602\routing\router;
use an602\symfony_request;
use an602\template\template;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\HttpFoundation\Cookie;

/**
 * A duplicate of \an602\controller\helper
 *
...
class helper
{
	 ...
	 * @param request				$an602_request
	 * @param symfony_request		$request
	 * @param router				$router
	 * @param string				$an602_root_path
	 */
	public function __construct(config $config, language $language, language_file_helper $lang_helper, navigation_provider $nav, template $template, path_helper $path_helper, request $an602_request, symfony_request $request, router $router, $an602_root_path)
	{
		...
		$this->an602_request = $an602_request;
	...
	public function handle_language_select()
	{
		...
		$submit = $this->an602_request->variable('change_lang', '');
		if (!empty($submit))
		{
			$lang = $this->an602_request->variable('language', '');
		}

		// Retrieve language from cookie
		$lang_cookie = $this->an602_request->variable('lang', '', false, request_interface::COOKIE);
	 ...
	 *
	 * @param \an602\install\helper\iohandler\iohandler_interface|null	$iohandler
	 */
