...
class file_updater implements file_updater_interface
{
	 ...
	 */
	public function __construct(filesystem $filesystem, $an602_root_path)
	{
	...
	{
		$this->filesystem->remove($this->an602_root_path . $path_to_file);
	}
	...
	{
		$path_to_file_to_create = $this->an602_root_path . $path_to_file_to_create;
		...
		if (!$this->filesystem->is_writable($dir))
		{
			...
			$this->filesystem->an602_chmod($dir, filesystem::CHMOD_ALL);
		}
		...
		{
			$this->filesystem->an602_chmod($dir, $original_dir_perms);
		}
	}
	...
	{
		$path_to_file_to_update = $this->an602_root_path . $path_to_file_to_update;
		...
		if (!$this->filesystem->is_writable($path_to_file_to_update))
		{
			...
			$this->filesystem->an602_chmod($path_to_file_to_update, filesystem::CHMOD_WRITE);
		}
		...
		{
			$this->filesystem->an602_chmod($path_to_file_to_update, $original_file_perms);
		}
	}
