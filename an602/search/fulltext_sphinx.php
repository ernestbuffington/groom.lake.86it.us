...
class fulltext_sphinx
{
	...
	/**
	 * Constructor
	 * Creates a new \an602\search\fulltext_postgres, which is used as a search backend
	 *
	 ...
	 */
	public function __construct(&$error, $an602_root_path, $phpEx, $auth, $config, $db, $user, $an602_dispatcher)
	{
		...
		// Initialize \an602\db\tools\tools object
		global $an602_container; // TODO inject into object
		...
		$this->indexes = 'index_an602_' . $this->id . '_delta;index_an602_' . $this->id . '_main';
	...
	*/
	...
	protected function config_generate()
	{
		...
		include($this->an602_root_path . 'config.' . $this->php_ext);
		...
		$config_object = new \an602\search\sphinx\config($this->config_file_data);
		$config_data = array(
			'source source_an602_' . $this->id . '_main' => array(
			...
			'source source_an602_' . $this->id . '_delta : source_an602_' . $this->id . '_main' => array(
				array('sql_query_pre',				'SET NAMES \'utf8\''),
			...
			'index index_an602_' . $this->id . '_main' => array(
				array('path',						$this->config['fulltext_sphinx_data_path'] . 'index_an602_' . $this->id . '_main'),
				array('source',						'source_an602_' . $this->id . '_main'),
			...
			'index index_an602_' . $this->id . '_delta : index_an602_' . $this->id . '_main' => array(
				array('path',						$this->config['fulltext_sphinx_data_path'] . 'index_an602_' . $this->id . '_delta'),
				array('source',						'source_an602_' . $this->id . '_delta'),
			),
			'indexer' => array(
		...
		*/
		...
		extract($this->an602_dispatcher->trigger_event('core.search_sphinx_modify_config_data', compact($vars)));
	...
	*/
	public function keyword_search($type, $fields, $terms, $sort_by_sql, $sort_key, $sort_dir, $sort_days, $ex_fid_ary, $post_visibility, $topic_id, $author_ary, $author_name, &$id_ary, &$start, $per_page)
	{
		...
		*/
		...
		extract($this->an602_dispatcher->trigger_event('core.search_sphinx_keywords_modify_options', compact($vars)));
		$this->sphinx = $sphinx;
		unset($sphinx);
		...
		{
			$an602_log->add('critical', $user->data['user_id'], $user->ip, 'LOG_SPHINX_ERROR', false, array($this->sphinx->GetLastError()));
	...
	*/
	...
	public function index($mode, $post_id, &$message, &$subject, $poster_id, $forum_id)
	{
		...
		*/
		...
		extract($this->an602_dispatcher->trigger_event('core.search_sphinx_index_before', compact($vars)));
