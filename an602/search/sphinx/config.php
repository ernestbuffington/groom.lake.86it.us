...
class config
{
	...
	* @return	\an602\search\sphinx\config_section			The section object or null if none was found
	*
	* @access	public
	*/
	function get_section_by_name($name)
	{
		for ($i = 0, $size = count($this->sections); $i < $size; $i++)
		{
			...
			if (($this->sections[$i] instanceof \an602\search\sphinx\config_section) && $this->sections[$i]->get_name() == $name)
			{
	...
	* @return	\an602\search\sphinx\config_section			The newly created section object
	*
	* @access	public
	*/
	function add_section($name)
	{
		$this->sections[] = new \an602\search\sphinx\config_section($name, '');
	...
	function read($config_data)
	{
		...
		foreach ($config_data as $i => $line)
		{
			...
			if (!$section)
			{
				...
				if (!$line || $line[0] == '#')
				{
					$this->sections[] = new \an602\search\sphinx\config_comment($config_file[$i]);
					continue;
				}
				else
				{
					...
					$section = new \an602\search\sphinx\config_section($section_name, $section_name_comment);
				}
			}
			else
			{
				...
				if (!$in_value)
				{
					...
					if (!$line || $line[0] == '#')
					{
						$section->add_variable(new \an602\search\sphinx\config_comment($config_file[$i]));
						continue;
					}
				...
				if ($line || $in_value)
				{
					...
					{
						$section->add_variable(new \an602\search\sphinx\config_variable(trim($name), trim($value), ($end_section) ? '' : $comment));
						continue;
					}
				...
				$section->add_variable(new \an602\search\sphinx\config_comment($comment));
			}
		}
