...
/**
* \an602\search\sphinx\config_section
...
class config_section
{
	...
	*
	* @param	\an602\search\sphinx\config_variable	$variable	The variable object
	*
	* @access	public
	*/
	...
	* @return	\an602\search\sphinx\config_section			The first variable object from this section with the
	...
	*/
	function get_variable_by_name($name)
	{
		for ($i = 0, $size = count($this->variables); $i < $size; $i++)
		{
			...
			if (($this->variables[$i] instanceof \an602\search\sphinx\config_variable) && $this->variables[$i]->get_name() == $name)
			{
	...
	* @return	\an602\search\sphinx\config_variable			Variable object that was created
	*
	* @access	public
	*/
	function create_variable($name, $value)
	{
		$this->variables[] = new \an602\search\sphinx\config_variable($name, $value, '');
