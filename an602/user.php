...
*/
class user extends \an602\session
{
	/**
	...
	*
	* @param \an602\language\language	$lang			AN602's Language loader
	* @param string						$datetime_class	Class name of datetime class
	*/
	function __construct(\an602\language\language $lang, $datetime_class)
	{
		global $an602_root_path;

		$this->lang_path = $an602_root_path . 'language/';
		$this->language = $lang;
	...
	*/
	...
	{
		global $db, $request, $template, $config, $auth, $phpEx, $an602_root_path, $cache;
		...
		else
		{
			...
			{
				$lang_override = $request->variable($config['cookie_name'] . '_lang', '', true, \an602\request\request_interface::COOKIE);
			}
		...
		*/
		...
		extract($an602_dispatcher->trigger_event('core.user_setup', compact($vars)));
		...
		// Call an602_user_session_handler() in case external application want to "bend" some variables or replace classes...
		// After calling it we continue script execution...
		an602_user_session_handler();
		...
		* @since 3.1.6-RC1
		*/
		$an602_dispatcher->dispatch('core.user_setup_after');
		...
		if (!$an602_container->getParameter('allow_install_dir') && !defined('ADMIN_START') && !defined('IN_INSTALL') && !defined('IN_LOGIN') && file_exists($an602_root_path . 'install') && !is_file($an602_root_path . 'install'))
		{
		...
		if (!defined('IN_ADMIN') && !defined('ADMIN_START') && $config['chg_passforce'] && !empty($this->data['is_registered']) && $auth->acl_get('u_chgpasswd') && $this->data['user_passchg'] < time() - ($config['chg_passforce'] * 86400))
		{
			...
			{
				redirect(append_sid("{$an602_root_path}ucp.$phpEx", 'i=profile&amp;mode=reg_details'));
			}
		}
	...
	* Function to mimic sprintf() with the possibility of using AN602's language system to substitute nullar/singular/plural forms.
	...
	*/
	function format_date($gmepoch, $format = false, $forcedate = false)
	{
		...
		*/
		...
		extract($an602_dispatcher->trigger_event('core.user_format_date_override', compact($vars)));
	...
	/**
	* Create a \an602\datetime object in the context of the current user
	*
	* @since 3.1
	* @param string $time String in a format accepted by strtotime().
	* @param DateTimeZone $timezone Time zone of the time.
	* @return \an602\datetime Date time object linked to the current users locale
	*/
	...
	*/
	function optionget($key, $data = false)
	{
		...
		return an602_optionget($this->keyoptions[$key], $var);
	}
	...
	*/
	function optionset($key, $value, $data = false)
	{
		...
		$new_var = an602_optionset($this->keyoptions[$key], $value, $var);
