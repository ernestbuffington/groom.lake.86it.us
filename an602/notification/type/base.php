...
*/
abstract class base implements \an602\notification\type\type_interface
{
	/** @var \an602\notification\manager */
	...
	*/
	 ...
	 */
	public function __construct(\an602\db\driver\driver_interface $db, \an602\language\language $language, \an602\user $user, \an602\auth\auth $auth, $an602_root_path, $php_ext, $user_notifications_table)
	{
		$this->db = $db;
	...
	*/
	public function set_notification_manager(\an602\notification\manager $notification_manager)
	{
	...
	*/
	public function prepare_for_display()
	{
		...
		if ($this->get_url())
		{
			$u_mark_read = append_sid($this->an602_root_path . 'index.' . $this->php_ext, 'mark_notification=' . $this->notification_id . '&amp;hash=' . $mark_hash);
		}
		else
		{
			...
			$u_mark_read = append_sid($this->an602_root_path . 'index.' . $this->php_ext, 'mark_notification=' . $this->notification_id . '&amp;hash=' . $mark_hash . '&amp;redirect=' . urlencode($redirect));
		}
