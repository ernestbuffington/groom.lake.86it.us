...
use an602\filesystem\exception\filesystem_exception;
...
{
	/**
	 * Store some information about file ownership for AN602's chmod function
	 *
	 * @var array
	 */
	...
	public function is_writable($files, $recursive = false)
	{
		if (defined('PHP_WINDOWS_VERSION_MAJOR') || !function_exists('is_writable'))
		{
			foreach ($this->to_iterator($files) as $file)
			{
				...
				if (!$this->an602_is_writable($file))
				{
					return false;
				}
			}
	 ...
	 * {@inheritdoc}
	 */
	public function an602_chmod($files, $perms = null, $recursive = false, $force_chmod_link = false)
	{
		if (is_null($perms))
		{
			// Default to read permission for compatibility reasons
	...
	public function realpath($path)
	{
		...
		{
			return $this->an602_own_realpath($path);
		}
	...
	/**
	 * AN602's implementation of is_writable
	 *
	 ...
	 */
	protected function an602_is_writable($file)
	{
		if (file_exists($file))
		{
		...
		else
		{
			...
			if (file_exists($dir) && is_dir($dir) && $this->an602_is_writable($dir))
			{
				return true;
			}
		}
	 ...
	 * @return bool|string
	 */
	protected function an602_own_realpath($path)
	{
		...
		else
		{
			...
			if (is_null($this->working_directory))
			{
				...
				{
					//
					// Assuming that the working directory is AN602 root
					// we could use this as a fallback, when AN602 will use controllers
