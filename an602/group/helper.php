...
namespace an602\group;

use an602\auth\auth;
use an602\cache\service as cache;
...
class helper
{
	...
	public function __construct(auth $auth, cache $cache, config $config, language $language, dispatcher_interface $dispatcher, path_helper $path_helper, user $user)
	{
		...
		$this->name_strings = array(
			'base_url'				=> "{$path_helper->get_an602_root_path()}memberlist.{$path_helper->get_php_ext()}?mode=group&amp;g={GROUP_ID}",
	...
	public function get_rank($group_data)
	{
		...
		if (!empty($group_data['group_rank']))
		{
			...
			if (isset($ranks['special'][$group_data['group_rank']]))
			{
				...
				$group_rank_data['img_src'] = (!empty($rank['rank_image'])) ? $this->path_helper->update_web_root_path($this->an602_root_path . $this->config['ranks_path'] . '/' . $rank['rank_image']) : '';
	...
	/**
	 * Get group avatar.
	 * Wrapper function for an602_get_group_avatar()
	 *
	...
	{
		return an602_get_group_avatar($group_row, $alt, $ignore_config, $lazy);
	}
}
