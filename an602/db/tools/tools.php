...
class tools implements tools_interface
{
	...
	*/
	public function __construct(\an602\db\driver\driver_interface $db, $return_statements = false)
	{
		$this->db = $db;
...
//							$sqlite_data[$table]['change_columns'][] = $result;
	...
	*/
	public function get_existing_indexes($table_name, $column_name, $unique = false)
	{
		...
		{
			case 'oracle':
				$sql = "SELECT ix.index_name  AS an602_index_name, ix.uniqueness AS is_unique
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			{
				$existing_indexes[$row['an602_index_name']] = array();
			}
		}
		$this->db->sql_freeresult($result);
		...
		{
			case 'oracle':
				$sql = "SELECT index_name AS an602_index_name, column_name AS an602_column_name
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			$existing_indexes[$row['an602_index_name']][] = $row['an602_column_name'];
		}
		$this->db->sql_freeresult($result);
