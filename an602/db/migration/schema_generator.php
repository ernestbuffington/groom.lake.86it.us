...
class schema_generator
{
	...
	* Constructor
	*/
	public function __construct(array $class_names, \an602\config\config $config, \an602\db\driver\driver_interface $db, \an602\db\tools\tools_interface $db_tools, $an602_root_path, $php_ext, $an602_table_prefix)
	{
		$this->config = $config;
		$this->db = $db;
	...
	public function get_schema()
	{
		...
		while (!empty($migrations))
		{
			foreach ($migrations as $key => $migration_class)
			{
				...
				if (\an602\db\migrator::is_migration($migration_class) === false)
				{
				...
				{
					$migration = new $migration_class($this->config, $this->db, $this->db_tools, $this->an602_root_path, $this->php_ext, $this->table_prefix);
