...
*/
class permission implements \an602\db\migration\tool\tool_interface
{
	/** @var \an602\auth\auth */
	protected $auth;
	...
	*/
	public function __construct(\an602\db\driver\driver_interface $db, \an602\cache\service $cache, \an602\auth\auth $auth, $an602_root_path, $php_ext)
	{
		$this->db = $db;
		$this->cache = $cache;
		$this->auth = $auth;
	...
	public function permission_set($name, $auth_option, $type = 'role', $has_permission = true)
	{
		...
		switch ($type)
		{
			...
			case 'group':
				...
				if (!$group_id)
				{
					throw new \an602\db\migration\exception('GROUP_NOT_EXIST', $name);
				}
				...
				if ($role_id)
				{
					...
					if (!$role_data)
					{
						throw new \an602\db\migration\exception('ROLE_ASSIGNED_NOT_EXIST', $name, $role_id);
					}
