...
namespace an602\db\migration\tool;
...
*/
class module implements \an602\db\migration\tool\tool_interface
{
	...
	/** @var \an602\module\module_manager */
	...
	* @param \an602\cache\service $cache
	* @param \an602\user $user
	* @param \an602\module\module_manager	$module_manager
	* @param string $an602_root_path
	* @param string $php_ext
	* @param string $modules_table
	*/
	public function __construct(\an602\db\driver\driver_interface $db, \an602\cache\service $cache, \an602\user $user, \an602\module\module_manager $module_manager, $an602_root_path, $php_ext, $modules_table)
	{
		$this->db = $db;
		$this->cache = $cache;
		$this->user = $user;
	...
	* @return null
	* @throws \an602\db\migration\exception
	*/
	public function add($class, $parent = 0, $data = array())
	{
		...
		foreach ($parents as $parent)
		{
			...
			{
				throw new \an602\db\migration\exception('MODULE_NOT_EXIST', $parent);
			}

			if ($this->exists($class, $parent, $data['module_langname']))
			{
				throw new \an602\db\migration\exception('MODULE_EXISTS', $data['module_langname']);
			}
			...
			try
			{
				...
				$an602_log->add('admin', (isset($user->data['user_id'])) ? $user->data['user_id'] : ANONYMOUS, $user->ip, 'LOG_MODULE_ADD', false, array($module_log_name));
				...
				if (isset($data['before']) && $data['before'])
				{
					...
					{
						// Restore legacy-legacy behaviour from AN602 3.0
						list($before_mode, $before_langname) = $data['before'];
					}
					else
					{
						// Legacy behaviour from AN602 3.1+
			...
			{
				// Error
				throw new \an602\db\migration\exception('MODULE_ERROR', $e->getMessage());
			}
		}
	...
	*/
	public function remove($class, $parent = 0, $module = '')
	{
		...
		if (is_array($module))
		{
			...
			{
				throw new \an602\db\migration\exception('MODULE_NOT_EXIST');
			}
	...
	*/
	protected function get_module_info($class, $basename)
	{
		...
		if (empty($module))
		{
			throw new \an602\db\migration\exception('MODULE_INFO_FILE_NOT_EXIST', $class, $basename);
		}
		...
		if (!is_numeric($parent_id))
		{
			...
			switch (count($ids))
			{
				...
				case 0:
					if ($throw_exception)
					{
						throw new \an602\db\migration\exception('MODULE_NOT_EXIST', $parent_id);
					}
				...
				default:
					// This represents the old behaviour of AN602 3.0
					return $ids;
				break;
			}
		}
