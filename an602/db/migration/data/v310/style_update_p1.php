...
class style_update_p1 extends \an602\db\migration\migration
{
	...
	public function styles_update()
	{
		...
		$iterator = new \DirectoryIterator($this->an602_root_path . 'styles');
		...
		foreach ($iterator as $fileinfo)
		{
			if ($fileinfo->isDir() && !in_array($fileinfo->getFilename(), $skip_dirs) && file_exists($fileinfo->getPathname() . '/style.cfg'))
			{
				...
				if (isset($style_cfg['an602_version']) && version_compare($style_cfg['an602_version'], '3.1.0-dev', '>='))
				{
					// 3.1 style
