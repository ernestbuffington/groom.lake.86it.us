...
class notification_options_reconvert extends \an602\db\migration\migration
{
	...
	{
		$insert_buffer = new \an602\db\sql_insert_buffer($this->db, $this->table_prefix . 'user_notifications');
	...
	*
	* @param \an602\db\sql_insert_buffer		$insert_buffer
	* @param int			$start		Start of staggering step
	* @return		mixed		int start of the next step, null if the end was reached
	*/
	public function perform_conversion(\an602\db\sql_insert_buffer $insert_buffer, $start)
	{
		$limit = 250;
		$converted_users = 0;
		$start = $start ?: 0;
	...
	*/
	protected function add_method_rows(\an602\db\sql_insert_buffer $insert_buffer, $item_type, $item_id, $user_id, array $methods)
	{
		$row_base = array(
			'item_type'		=> $item_type,
			'item_id'		=> (int) $item_id,
