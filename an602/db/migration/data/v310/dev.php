...
class dev extends \an602\db\migration\container_aware_migration
{
	...
	{
		return array(
			'\an602\db\migration\data\v310\extensions',
			'\an602\db\migration\data\v310\style_update_p2',
			'\an602\db\migration\data\v310\timezone_p2',
			'\an602\db\migration\data\v310\reported_posts_display',
			'\an602\db\migration\data\v310\migrations_table',
		);
	}
	...
	public function update_data()
	{
		return array(
			array('if', array(
				(strpos('an602_search_', $this->config['search_type']) !== 0),
				array('config.update', array('search_type', 'an602_search_' . $this->config['search_type'])),
			)),
	...
	public function move_customise_modules()
	{
		...
		{
			include($this->an602_root_path . 'includes/functions_acp.' . $this->php_ext);
		}
	...
	public function rename_module_basenames()
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			{
				// all the class names start with class name or with an602_ for auto loading
				if (strpos($row['module_basename'], $row['module_class'] . '_') !== 0 &&
					strpos($row['module_basename'], 'an602_') !== 0)
				{
