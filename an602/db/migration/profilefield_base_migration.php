...
abstract class profilefield_base_migration extends container_aware_migration
{
	...
	*/
	...
	public function create_custom_field()
	{
		...
		$result = $this->db->sql_query($sql);
		$lang_name = (strpos($this->profilefield_name, 'an602_') === 0) ? strtoupper(substr($this->profilefield_name, 6)) : strtoupper($this->profilefield_name);
	...
	*/
	public function create_language_entries()
	{
		...
		$insert_buffer = new \an602\db\sql_insert_buffer($this->db, AN602_PROFILE_FIELDS_LANG_TABLE);
	...
	*/
	...
	{
		$insert_buffer = new \an602\db\sql_insert_buffer($this->db, $this->table_prefix . 'profile_fields_data');
		$limit = 250;
		$converted_users = 0;
		$start = $start ?: 0;
	...
	protected function get_insert_sql_array()
	{
		...
		if ($profile_row === null)
		{
			/* @var $manager \an602\profilefields\manager */
