...
use an602\db\output_handler\migrator_output_handler_interface;
use an602\db\output_handler\null_migrator_output_handler;
...
class migrator
{
	...
	*/
	protected function try_apply($name)
	{
		...
		foreach ($state['migration_depends_on'] as $depend)
		{
			...
			if ($missing !== false)
			{
				throw new \an602\db\migration\exception('MIGRATION_NOT_FULFILLABLE', $name, $missing);
			}
	...
	protected function get_callable_from_step(array $step, $last_result = 0, $reverse = false)
	{
		...
		switch ($class)
		{
			case 'if':
				...
				{
					throw new \an602\db\migration\exception('MIGRATION_INVALID_DATA_MISSING_CONDITION', $step);
				}

				if (!isset($parameters[1]))
				{
					throw new \an602\db\migration\exception('MIGRATION_INVALID_DATA_MISSING_STEP', $step);
				}
			...
			case 'custom':
				...
				{
					throw new \an602\db\migration\exception('MIGRATION_INVALID_DATA_CUSTOM_NOT_CALLABLE', $step);
				}
			...
			default:
				if (!$method)
				{
					throw new \an602\db\migration\exception('MIGRATION_INVALID_DATA_UNKNOWN_TYPE', $step);
				}

				if (!isset($this->tools[$class]))
				{
					throw new \an602\db\migration\exception('MIGRATION_INVALID_DATA_UNDEFINED_TOOL', $step);
				}

				if (!method_exists(get_class($this->tools[$class]), $method))
				{
					throw new \an602\db\migration\exception('MIGRATION_INVALID_DATA_UNDEFINED_METHOD', $step);
				}
	...
	*
	* @param string $name Name of the migration
	* @return \an602\db\migration\migration
	*/
	public function get_migration($name)
	{
		$migration = new $name($this->config, $this->db, $this->db_tools, $this->an602_root_path, $this->php_ext, $this->table_prefix);
	...
	*/
	...
	static public function is_migration($migration)
	{
		if (class_exists($migration))
		{
			...
			// an602\db\migration\migration (which implements the
			// migration_interface) and be instantiable.
			$reflector = new \ReflectionClass($migration);
			if ($reflector->implementsInterface('\an602\db\migration\migration_interface') && $reflector->isInstantiable())
			{
				return true;
			}
		}
