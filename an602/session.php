...
class session
{
	 ...
	 *
	 * @param string $root_path current root path (an602_root_path)
	 * @return array
	 */
	static function extract_current_page($root_path)
	{
		global $request, $symfony_request, $an602_filesystem;
		...
		$symfony_request_path = $an602_filesystem->clean_path($symfony_request->getPathInfo());
		...
		{
			// current directory within the AN602 root (for example: adm)
			$root_dirs = explode('/', str_replace('\\', '/', $an602_filesystem->realpath($root_path)));
			$page_dirs = explode('/', str_replace('\\', '/', $an602_filesystem->realpath('./')));
		}
		...
		// We are on the base level (AN602 root == webroot), lets adjust the variables a bit...
		if (!$root_script_path)
		{
	...
	*/
	...
	{
		global $phpEx, $SID, $_SID, $_EXTRA_URL, $db, $config, $an602_root_path;
		global $request, $an602_container, $user, $an602_log, $an602_dispatcher;
		...
		$this->page					= $this->extract_current_page($an602_root_path);
		...
		if ($request->is_set($config['cookie_name'] . '_sid', \an602\request\request_interface::COOKIE) || $request->is_set($config['cookie_name'] . '_u', \an602\request\request_interface::COOKIE))
		{
			$this->cookie_data['u'] = $request->variable($config['cookie_name'] . '_u', 0, false, \an602\request\request_interface::COOKIE);
			$this->cookie_data['k'] = $request->variable($config['cookie_name'] . '_k', '', false, \an602\request\request_interface::COOKIE);
			$this->session_id 		= $request->variable($config['cookie_name'] . '_sid', '', false, \an602\request\request_interface::COOKIE);
		...
		*/
		$vars = array('ip');
		extract($an602_dispatcher->trigger_event('core.session_ip_after', compact($vars)));
...
//				$quadcheck = ($config['ip_check_bot'] && $this->data['user_type'] & USER_BOT) ? 4 : $config['ip_check'];
				...
				if ($u_ip === $s_ip && $s_browser === $u_browser && $s_forwarded_for === $u_forwarded_for && $referer_valid)
				{
					...
					if (!($provider instanceof \an602\auth\provider\provider_interface))
					{
						throw new \RuntimeException($provider . ' must implement \an602\auth\provider\provider_interface');
					}
				...
				else
				{
					...
					if ($an602_container->getParameter('session.log_errors') && $this->data['user_id'] != ANONYMOUS)
					{
						if ($referer_valid)
						{
							$an602_log->add('critical', $user->data['user_id'], $user->ip, 'LOG_IP_BROWSER_FORWARDED_CHECK', false, array(
								$u_ip,
								$s_ip,
								$u_browser,
								$s_browser,
						...
						{
							$an602_log->add('critical', $user->data['user_id'], $user->ip, 'LOG_REFERER_INVALID', false, array($this->referer));
						}
					}
				}
			}
	...
	*/
	...
	{
		global $SID, $_SID, $db, $config, $cache, $an602_container, $an602_dispatcher;
...
//		$db->sql_return_on_error(false);
		...
		// We are not able to simply have an empty session_page btw, therefore we need to tell AN602 how to detect this special case.
		...
		*/
		...
		extract($an602_dispatcher->trigger_event('core.session_create_after', compact($vars)));
		unset($session_data);
	...
	*/
	function session_kill($new_session = true)
	{
		...
		*/
		...
		extract($an602_dispatcher->trigger_event('core.session_kill_after', compact($vars)));
		unset($user_id);
		unset($session_id);
	...
	*/
	function session_gc()
	{
		...
		{
			case 'sqlite3':
				if (an602_version_compare($db->sql_server_info(true), '3.8.3', '>='))
				{
		...
		/* @var \an602\captcha\factory $captcha_factory */
		...
		* @since 3.1.6-RC1
		*/
		$an602_dispatcher->dispatch('core.session_gc_after');
	...
	*
	* @param string $name		Name of the cookie, will be automatically prefixed with the AN602 cookie name. track becomes [cookie_name]_track then.
	...
	*/
	function set_cookie($name, $cookiedata, $cookietime, $httponly = true)
	{
		...
		*/
		...
		extract($an602_dispatcher->trigger_event('core.set_cookie', compact($vars)));
	...
	*/
	...
	{
		global $config, $db, $an602_dispatcher;
		...
		*/
		...
		extract($an602_dispatcher->trigger_event('core.session_set_custom_ban', compact($vars)));
...
//				$this->session_create(ANONYMOUS);
			...
			$contact_link = an602_get_board_contact_link($config, $an602_root_path, $phpEx);
	...
	*/
	function set_login_key($user_id = false, $key = false, $user_ip = false)
	{
		...
		extract($an602_dispatcher->trigger_event('core.set_login_key', compact($vars)));
	...
	*/
	public function update_session($session_data, $session_id = null)
	{
		...
		*/
		...
		extract($an602_dispatcher->trigger_event('core.update_session_after', compact($vars)));
	}
