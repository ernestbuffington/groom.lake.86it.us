...
abstract class captcha_abstract
{
	...
	*/
	...
	{
		global $config, $user, $template, $phpEx, $an602_root_path;
		...
		{
			$link = append_sid($an602_root_path . 'ucp.' . $phpEx,  'mode=confirm&amp;confirm_id=' . $this->confirm_id . '&amp;type=' . $this->type);
	...
	{
		global $config, $template, $request, $an602_admin_path, $phpEx;
		...
		$template->assign_vars(array(
			'CONFIRM_IMAGE'		=> append_sid($an602_admin_path . 'index.' . $phpEx, 'captcha_demo=1&amp;mode=visual&amp;i=' . $id . '&amp;select_captcha=' . $this->get_service_name()) . $variables,
			'CONFIRM_ID'		=> $this->confirm_id,
		));
