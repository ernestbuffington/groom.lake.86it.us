...
*/
class file extends \an602\cache\driver\base
{
	...
	* Set cache path
	*
	* @param string $cache_dir Define the path to the cache directory (default: $an602_root_path . 'cache/')
	*/
	function __construct($cache_dir = null)
	{
		global $an602_container;

		$this->cache_dir = !is_null($cache_dir) ? $cache_dir : $an602_container->getParameter('core.cache_dir');
		$this->filesystem = new \an602\filesystem\filesystem();
	...
	*/
	function _write($filename, $data = null, $expires = 0, $query = '')
	{
		...
		$lock = new \an602\lock\flock($file);
		$lock->acquire();
		...
		if ($handle = @fopen($file, 'wb'))
		{
			...
			try
			{
				$this->filesystem->an602_chmod($file, \an602\filesystem\filesystem_interface::CHMOD_READ | \an602\filesystem\filesystem_interface::CHMOD_WRITE);
			}
