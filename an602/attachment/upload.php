...
class upload
{
	...
	/** @var \an602\files\upload Upload class */
	...
	/** @var \an602\files\filespec Current filespec instance */
	private $file;
	 ...
	 * @param service $cache
	 * @param config $config
	 * @param \an602\files\upload $files_upload
	 * @param language $language
	 * @param guesser $mimetype_guesser
	 * @param dispatcher $an602_dispatcher
	 * @param plupload $plupload
	 * @param user $user
	 * @param $an602_root_path
	 */
	public function __construct(auth $auth, service $cache, config $config, \an602\files\upload $files_upload, language $language, guesser $mimetype_guesser, dispatcher $an602_dispatcher, plupload $plupload, user $user, $an602_root_path)
	{
		$this->auth = $auth;
		$this->cache = $cache;
		$this->config = $config;
	...
	public function upload($form_name, $forum_id, $local = false, $local_storage = '', $is_message = false, $local_filedata = array())
	{
		...
		extract($this->an602_dispatcher->trigger_event('core.modify_uploaded_file', compact($vars)));
	...
	protected function check_disk_space()
	{
		...
		{
			$free_space = @disk_free_space($this->an602_root_path);
