...
class delete
{
	 ...
	 */
	public function __construct(config $config, driver_interface $db, dispatcher $dispatcher, filesystem $filesystem, resync $resync, $an602_root_path)
	{
		$this->config = $config;
		$this->db = $db;
	...
	public function unlink_attachment($filename, $mode = 'file', $entry_removed = false)
	{
		...
		$filepath = $this->an602_root_path . $this->config['upload_path'] . '/' . $filename;

		try
		{
			if ($this->filesystem->exists($filepath))
			{
				$this->filesystem->remove($this->an602_root_path . $this->config['upload_path'] . '/' . $filename);
				return true;
			}
		}
		catch (\an602\filesystem\exception\filesystem_exception $exception)
		{
