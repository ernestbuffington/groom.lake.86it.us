...
class user_loader
{
	...
	/** @var string */
	protected $an602_root_path = null;
	...
	*/
	public function __construct(\an602\db\driver\driver_interface $db, $an602_root_path, $php_ext, $users_table)
	{
		$this->db = $db;
	...
	*/
	public function get_avatar($user_id, $query = false, $lazy = false)
	{
		...
		return an602_get_avatar($row, 'USER_AVATAR', false, $lazy);
	}
	...
	*/
	public function get_rank($user_id, $query = false)
	{
		...
		$user_rank_data = an602_get_user_rank($user, (($user['user_id'] == ANONYMOUS) ? false : $user['user_posts']));
