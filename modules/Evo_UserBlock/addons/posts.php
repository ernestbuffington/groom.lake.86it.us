...
function evouserinfo_posts() 
{
	...
	aria-hidden="true"></i></font>&nbsp;'.$titanium_lang_evo_userblock['BLOCK']['POSTS']['POSTS'].'<span style="float:right">'.number_format($phpbb2_posts).'&nbsp;&nbsp;</span>';
    $evouserinfo_posts .= '</div>';

    $evouserinfo_posts .= '<div style="padding-left: 10px;">';
    $evouserinfo_posts .= '  <font color="yellow"><i class="fas fa-chart-bar" 
	aria-hidden="true"></i></font>&nbsp;'.$titanium_lang_evo_userblock['BLOCK']['POSTS']['TOPICS'].'<span style="float:right">'.number_format($phpbb2_topics).'&nbsp;&nbsp;</span>';
    $evouserinfo_posts .= '</div>';
    ...
    if (is_user()):
	  ...
	  aria-hidden="true"></i></font>&nbsp;'.$titanium_lang_evo_userblock['BLOCK']['POSTS']['UR_TOPICS'].'<span style="float:right">'.evouserinfo_ur_total_topics().'&nbsp;&nbsp;</span>';
      $evouserinfo_posts .= '</div>';

      $evouserinfo_posts .= '<div style="padding-left: 10px;">';
      $evouserinfo_posts .= '  <font color="orange"><i class="fas fa-chart-bar" 
	  aria-hidden="true"></i></font>&nbsp;<a href="modules.php?name=Forums&file=search&search_id=egosearch">'.$titanium_lang_evo_userblock['BLOCK']['POSTS']['UR_POSTS'].
