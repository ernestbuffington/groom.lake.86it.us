...
class mcp_socialnet
{
	...
	function mcp_socialnet(&$p_master)
	{
	...
	{
		global $config, $db, $user, $auth, $template, $an602_root_path, $phpEx, $socialnet_root_path;
		
		$user->add_lang( array('mods/socialnet', 'mods/socialnet_acp'));
		$this->tpl_name = 'socialnet/mcp_socialnet';
		$this->page_title = 'MCP_SOCIALNET';

		$mcpPanel_title = 'MCP_SN_' . strtoupper(preg_replace('/^module_/si', '', $mode));
	...
	foreach ($config_vars as $config_name => $config_definition)
	{
		...
		switch ($validator[$type])
		{
			...
			case 'wpath':
				...
				if (!file_exists($an602_root_path . $cfg_array[$config_name]))
				{
					$error[] = sprintf($user->lang['DIRECTORY_DOES_NOT_EXIST'], $cfg_array[$config_name]);
				}

				if (file_exists($an602_root_path . $cfg_array[$config_name]) && !is_dir($an602_root_path . $cfg_array[$config_name]))
				{
				...
				{
					if (file_exists($an602_root_path . $cfg_array[$config_name]) && !@is_writable($an602_root_path . $cfg_array[$config_name]))
					{
