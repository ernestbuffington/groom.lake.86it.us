...
function change_topic_type($action, $topic_ids)
{
	...
	$forum_id = an602_check_ids($topic_ids, TOPICS_TABLE, 'topic_id', $check_acl, true);
...
function mcp_move_topic($topic_ids)
{
	...
	$forum_id = an602_check_ids($topic_ids, TOPICS_TABLE, 'topic_id', array('m_move'), true);
...
function mcp_restore_topic($topic_ids)
{
	...
	if (!an602_check_ids($topic_ids, TOPICS_TABLE, 'topic_id', array('m_approve')))
	{
		return;
	}
...
function mcp_delete_topic($topic_ids, $is_soft = false, $soft_delete_reason = '', $action = 'delete_topic')
{
	...
	*/
	...
	if (!an602_check_ids($topic_ids, TOPICS_TABLE, 'topic_id', $check_permission))
	{
		return;
	}
...
function mcp_delete_post($post_ids, $is_soft = false, $soft_delete_reason = '', $action = 'delete_post')
{
	...
	$check_permission = ($is_soft) ? 'm_softdelete' : 'm_delete';
	if (!an602_check_ids($post_ids, POSTS_TABLE, 'post_id', array($check_permission)))
	{
		return;
	}
...
function mcp_fork_topic($topic_ids)
{
	...
	if (!an602_check_ids($topic_ids, TOPICS_TABLE, 'topic_id', array('m_')))
	{
		return;
	}
