...
class acp_socialnet extends AddOnsHookSystem
{
	...
	function main($id, $mode)
	{
		...
		global $an602_root_path, $an602_admin_path, $phpEx, $socialnet_root_path;
		...
		$u_action_start = append_sid($an602_admin_path . 'index.' . $phpEx, "i={$id}&amp;mode={$mode}");
		...
		$template_assign_vars = array(
			...
			'S_MODE'					 => $mode,
			'U_FIND_USERNAME'			 => append_sid("{$an602_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=acp_socialnet_sn_basictools&amp;field=username&amp;select_single=true'),
	 ...
	 * @param integer $id AN602 variable
	 * @param string $module  Which acp panel module should be loaded into AN602 ACP
	 */
	function module($id, $module)
	{
		...
		{
			$module_acp_filename = str_replace($an602_root_path, '/', $module_acp_filename);
			$access_module = false;
		...
		{
			$module_filename = str_replace($an602_root_path, '/', $module_filename);
	...
	{
		global $config, $an602_root_path, $phpEx, $template, $user;
		...
		foreach ($avail_blocks as $block_name => $enable)
		{
			...
			$block_acpfile = "{$an602_root_path}socialnet/acp/{$blockName}.{$phpEx}";
	...
	function _settings($id, $mode, $display_vars = array())
	{
		...
		global $config, $an602_root_path, $an602_admin_path, $phpEx;
		global $cache;
...
class AddOnsHookSystem
{
	...
	{
		global $template, $an602_root_path, $phpEx;

		$addon_folder = "{$an602_root_path}socialnet/addons/";
	...
	function _templates()
	{
		global $template, $an602_root_path, $user;

		$template_short_dir = 'socialnet/addons/';
		$template_dir = "{$an602_root_path}styles/{$user->theme['template_path']}/template/{$template_short_dir}";
		$dir = @opendir($template_dir);
		$available_template_dir = false;
		if (!$dir)
		{
			$template_dir = "{$an602_root_path}styles/prosilver/template/{$template_short_dir}";
			$dir = opendir($template_dir);
