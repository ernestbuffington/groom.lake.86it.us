...
namespace an602\skeleton\controller;
...
use an602\skeleton\helper\packager;
use an602\skeleton\helper\validator;
...
class main
{
	...
	public function __construct(config $config, helper $helper, language $language, request $request, packager $packager, validator $validator, template $template, user $user)
	{
		...
		$this->language->add_lang('common', 'an602/skeleton');
	}
	...
	public function handle()
	{
		...
		$this->template->assign_var('S_POST_ACTION', $this->helper->route('an602_skeleton_controller'));

		return $this->helper->render('@an602_skeleton/skeleton_body.html', $this->language->lang('AN602_CREATE_SKELETON_EXT'));
	}
